...
def main_worker(gpu, ngpus_per_node, args):
    ...
    Score = test(test_loader, model, sr_model, perception_net, args)
...
def test(test_loader, model, sr_model, perception_net, args):
    ...
    for i, (input, target, post_path) in enumerate(test_loader):     
        ...
        # calculate tp, fp, fn, tn with 0 label as positive
        tp += sum((target==pred)*(0==pred))
        fp += sum((target!=pred)*(0==pred))
        fn += sum((target!=pred)*(1==pred))
        tn += sum((target==pred)*(1==pred))
