...
# Define a function for one validation epoch
...
def main(args):
    ...
    noise_fake_test = array_from_imgdir(
        args.noise_fake_root
    )

    noise_real_test = array_from_imgdir(
        args.noise_real_root 
    )

    x_test = torch.cat([real_test, fake_test], dim=0)
    y_test = torch.tensor([0.0] * len(real_test) + [1.0] * len(fake_test))    
    noise_x_test = torch.cat([noise_real_test, noise_fake_test], dim=0)
    del real_test, fake_test, noise_real_test, noise_fake_test
    ...
    noise_x_test_tf = dct.dct_2d(noise_x_test, norm = 'ortho')
    noise_x_test_tf = torch.log(torch.abs(noise_x_test_tf) + 1e-12)

    x_test_tf = x_test_tf.squeeze(1)
    noise_x_test_tf = noise_x_test_tf.squeeze(1)

    x_test_tf = x_test_tf.reshape(x_test_tf.shape[0], -1)
    noise_x_test_tf = noise_x_test_tf.reshape(noise_x_test_tf.shape[0], -1)
    ...
    noise_means = torch.load(os.path.join(args.path_to_mean_std, 'noise_means.pt'))
    noise_stds = torch.load(os.path.join(args.path_to_mean_std, 'noise_stds.pt'))
    noise_x_test_tf = (noise_x_test_tf - noise_means) / noise_stds
    print("rescaled...")

    combined_x = torch.cat([x_test_tf, noise_x_test_tf], dim=1)

    test_dataset = MyDataset(combined_x, y_test)
