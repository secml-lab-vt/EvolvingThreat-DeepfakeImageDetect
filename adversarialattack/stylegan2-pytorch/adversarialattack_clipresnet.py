...
def loadOpenAICLIPResnet():
    model = OpenAICLIPModel("xxl")  
    model.load_state_dict(torch.load('<path to finetuned surrogate model>'))
    model.to(DEVICE)
    model.eval()
    return model
...
classifier = loadOpenAICLIPResnet()
