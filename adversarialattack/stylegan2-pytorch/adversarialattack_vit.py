...
def loadViTClassifier(model_path):
...
classifier, feature_extractor = loadViTClassifier("<path to surrogate model>")
        ...
        for xind, imgfile in enumerate(imgfiles):
            ...
            for tempi in range(OPTIMIZATION_ITERATION): 
                try:
                    ...
                    tensor0 = vit_transform(img_gen)
                    tensor1 = tensor0.clamp_(min=-1, max=1) 
                    tensor2 = tensor1.add(1) 
                    tensor3 = torch.squeeze(tensor2.div_(2))
                    img_gen_vit2 = feature_extractor(tensor3.cpu())["pixel_values"] 
                    output, temploss = classifier(torch.unsqueeze(img_gen_vit2[0].to(DEVICE), 0), None)
                    # print('output', output)  
